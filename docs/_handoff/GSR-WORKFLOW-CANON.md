...
**Basecamp = keep it, integrate it, two-way sync (Daniel, 2026-06-08).** Basecamp stays a first-class live source the team already uses; we do NOT ditch it. The dashboard integrates as much of the existing Basecamp system as practical via TWO-WAY sync (read Basecamp into the dashboard AND write dashboard changes back to Basecamp), so the two stay in agreement and the team can work from either side. The live production pipeline already lives in the **"02_ Production"** project (account **5805529**) as the **"Genesis Science Report"** card table (columns `Triage -> Not now -> Recorded -> In Progress -> Editing -> Rendering -> Done`) plus the **"WWN"** card table; staff scheduling + admin to-dos live in **"01_DRM Staff"**. The per-role data inventory (which Basecamp data feeds each of the 4 role dashboards, and sync direction) is in `docs/2026-06-08-basecamp-dashboard-integration.md`. Display/placement is intentionally left open. Write-back is a live-team action: any dashboard->Basecamp write follows confirm-before-write + the David rule. This SUPERSEDES the earlier "Basecamp = read-only, later monologue-ingestion only" framing (e.g. handoff §11, tools-curriculum timeline); monologue ingestion is now one slice of a broader two-way integration. Credentials verified working 2026-06-08 (`docs/2026-06-08-basecamp-env-diagnosis.md`).
