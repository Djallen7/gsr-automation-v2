...
- **Mac <-> dashboard job transport: the Mac/worker POLLS a Supabase `jobs` table.** The dashboard (Vercel) writes a job row to trigger heavy work; the Mac polls every few seconds, runs pending jobs, and writes status back. No inbound connection to the Mac (Tailscale stays off-limits for automation), no extra queue service. This is the control plane for all heavy media (uploads, transcription).
...
## 13. Security incident correction (Daniel + David, confirmed 2026-06-11)
