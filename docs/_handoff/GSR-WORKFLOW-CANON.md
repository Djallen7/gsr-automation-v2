...
**Graphic-type naming (Daniel, 2026-06-09):** standardize the first monologue/segment graphic's **Graphic Type as "Intro Graphic" across the board** — not "Title Graphic." The tracker and prior docs mix the two ("Intro Graphic | Title Graphic: '<name>'" vs "Title Graphic"); "Intro Graphic" is the canonical type label going forward. Apply in the June Pass-2 build, the extractor's cue #1 default, and any DB CHECK / type enum + docs that list graphic types (e.g. this file §4, PROMPT_LIBRARY, the graphics page UX). Tracked in `docs/AUTOMATION_ROADMAP.md`.

**Graphic-type standardization, round 2 (Daniel, 2026-06-09):** a clip with **no audio is classified as `B-roll`** — do not add a separate "no audio" type; note "silent / under VO" in the description. Priority is one standardized vocabulary across the whole pipeline (extractor, tracker, dashboard, DB CHECK), kept close to the terms Daniel already uses. Canonical monologue graphic-type set: Intro Graphic, Clip w/audio, B-roll, Pre-made: B-roll, Picture, Article Screenshot, Propres Quote, Propres Graphic, Graphic, Book Cover, Roll-in. Full extraction rules + cue→type mapping live in `docs/2026-06-09-monologue-graphics-extraction-spec.md` (the SSOT), consumed by the `gsr-monologue-graphics` subagent. NOTE: the S03 Ep001-005 trackers name additional graphics hands — **Anderson, Ethan, Bella** — confirm their roles for the crew registry.
