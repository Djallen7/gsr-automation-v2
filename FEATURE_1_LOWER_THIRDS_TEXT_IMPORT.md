...
## The extraction prompt (v2 — current)
...
show_intro, opening_monologue, interview_1, interview_2, kids_corner,
...
- They appear as standalone lines, sometimes prefixed with "LT:",
  "LOWER THIRD:", or "GRAPHIC:".
- A name + role pattern is common: "DR. JOHN SMITH / GEOLOGIST" → guest_chyron.
- Segment headers like "INTERVIEW 1", "KIDS CORNER", "MINISTRY REPORT"
  tell you which segment enum value to use.
- If a segment header is ambiguous, map it to the closest enum value
  and put the original header text in source_doc.
...
- primary MUST be 1..200 characters. If a line is longer, put it in the
  top-level "rejected" array (do not emit it in the graphics array).
...
For l3_type values episode_intro_l3, segment_graphics_title, and topic_l3,
include shorter variant versions in var_1 (medium) and var_2 (short). These
are used when the text must fit in a narrower ProPresenter slide slot.
For all other l3_types, omit var_1 and var_2.
