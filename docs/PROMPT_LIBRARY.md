...
Every prompt in this file is grounded in production data from 879 Claude sessions, the ChatGPT export, active Drive documents, and the Supabase schema. Rules are derived from what actually caused problems in production — not invented from scratch.

**Version:** 2026-05-28  
**Status:** DRAFT — each prompt requires Daniel's approval before adding to /toolkit

---

## How to Use

Copy the prompt text and paste it at the start of a new Claude or ChatGPT session. Replace `{BRACKETS}` with the specific value for your task. The `/toolkit` page in the dashboard will inject live Supabase data (guest roster, episode list) automatically for prompts that need it.
...
| # | Name | Category | Notes |
|---|------|----------|-------|
| 01 | Guest Research | Research | Live roster injection via /toolkit |
| 02 | Interview Questions | Script | 10–12 questions per guest, 4-arc structure |
| 03 | Episode Script Outline | Script | All 15 segments, fixed order |
| 04 | Opening Monologue Script | Script | Teleprompter format, David's voice |
| 05 | Lower Thirds — Interview Segments | Graphics | 55–65 chars, ALL CAPS, variants |
| 06 | Lower Thirds — Opening Monologue | Graphics | 15 beats × 3 variations + flag pills |
| 07 | Lower Thirds — Ministry Report | Graphics | 3 beats (E3/E4/E5), E5 locked |
| 08 | YouTube Titles | Metadata | 30%-shorter rule, segment titles |
| 09 | YouTube Description + Metadata | Metadata | Full description + tags + JSON |
| 10 | Cold Outreach Email (Tier 1–2) | Email | 8-block structure, hard rules |
| 11 | Returning Guest Outreach (Tier 3–5) | Email | Shorter, direct, tier-matched |
| 12 | Booking Confirmation | Email | 2-email sequence (confirm + Zoom) |
| 13 | Pre-Interview Reminder | Email | Talking points, bio, chyron, graphics |
| 14 | Post-Interview Thank-You | Email | Air date + caveat |
| 15 | No-Response Follow-Up | Email | 5–7 days after cold outreach |
| 16 | Graphics Assignment | Production | Isaac/Jakob/Jeremiah roles, per segment |
| 17 | Graphics Sourcing | Production | What to find/create per segment |
| 18 | Distribution Checklist | Post-Production | All 6 platforms, requirements |
| 19 | Rundown Creator Population | Production | Parse script cues → RC columns |
| 20 | Episode Metadata JSON | Post-Production | Full structured metadata for pipeline |

---

## 01 — Guest Research

**When to use:** Starting a session to find new interview guests.  
**Data injection:** The /toolkit page injects the live guest roster from Supabase automatically.

> **⚠️ Recurring error (from 298 guest research sessions):** Claude will suggest recently-aired guests unless the roster is injected explicitly. The /toolkit page handles this — do not skip the roster injection step.

```
You are helping me research guests for Genesis Science Report (GSR), a weekly Christian creation-science TV show hosted by David Rives. Season 3, ~58 min, weekly. Air day: Tuesday 8 PM CST.
...
**When to use:** Preparing questions for a scheduled guest interview.  
**Format:** 10–12 questions, ~13 min segment.

> **Source:** Beat structure derived from `GSR_May28-29_Script_Outlines.md` and 5 interview question docs found in Drive (Jake Hebert, Jason Lisle, Jeff Tomkins, David DeWitt, Joe Deweese, Tommy Lohman).

```
You are writing interview questions for Genesis Science Report (GSR), hosted by David Rives.
...
## 03 — Episode Script Outline

**When to use:** Planning a full episode's structure before writing individual segments.

> **Source:** Segment order and timings from `GSR_Shared_Folder_04_Schedules_New_Macro_Rundown.docx` and `2026_GSR_Episode_Titles_Timecodes_UPDATED`.

```
You are outlining a teleprompter script for Genesis Science Report (GSR), Season 3, hosted by David Rives. ~58 minutes total.
...
## 04 — Opening Monologue Script

**When to use:** Writing the scripted teleprompter copy for David's opening segment.

> **Source:** Season 2 blank teleprompter template structure. David's sign-off is verbatim from that document.

```
You are writing the Opening Monologue for Genesis Science Report (GSR), Season 3, hosted by David Rives. This is the TELEPROMPTER SCRIPT David reads directly to camera.
...
## 05 — Lower Thirds: Interview Segments

**When to use:** Generating on-screen graphics text for interview segments (Interview 1 or Interview 2).

> **⚠️ Recurring error (from 146 lower-thirds sessions):** Lines run long. Count every character before submitting. Show the count.

```
You are writing lower-third graphics text for Genesis Science Report (GSR) ProPresenter templates. ALL TEXT IS ALL CAPS. Count every character before finalizing.
...
## 06 — Lower Thirds: Opening Monologue

**When to use:** Generating lower-third graphics for the Opening Monologue segment.

> **Source:** `GSR_Lower_Thirds_Worksheet.pdf` structure — 15 beats × 3 variations + flag pills.

```
You are writing lower-third graphics text for Genesis Science Report (GSR) — Opening Monologue segment only.
...
## 07 — Lower Thirds: Ministry Report
...
**When to use:** Writing the main episode title and per-segment titles for YouTube.

> **⚠️ Recurring error (from 59+ YouTube title sessions):** Claude forgets the 30%-shorter rule without explicit restatement. This prompt embeds it.  
> **Source:** `gold_corpus_v3__YouTube_-_Season_2_YT_Title_Prompt.docx` + production title history.

```
You are writing YouTube titles for Genesis Science Report (GSR), Season 3.
...
## 09 — YouTube Description + Tags

**When to use:** Writing the full YouTube description and tag list for an episode.

> **Source:** Canonical description format reverse-engineered from S03 Ep008 and the YouTube metadata spreadsheet (Folder `07_Distribution/YouTube`).

```
You are writing the complete YouTube description and tags for a Genesis Science Report episode.
...
MONOLOGUE TOPIC: {TOPIC}
SPONSOR: {Cedarville University / none}
...
6. Sponsor line if applicable: "This Episode has been sponsored by {SPONSOR}." (Cedarville University was sponsor through Ep024; none after)
...
## 10 — Cold Outreach Email (Tier 1–2)

**When to use:** First-ever contact with a guest who has never appeared on GSR (Tier 1: no prior contact; Tier 2: no appearance but prior thread).

> **Source:** `GSR_Outreach_Email_Template_Guide.md` (Drive ID: `1A2wQswqKL32521xgUrPE9mfcVsvzqQJDlF6oK7EiKhw`). This is the most thoroughly documented workflow in the archive.

```
You are drafting a cold guest outreach email for Genesis Science Report (GSR).
...
## 11 — Returning Guest Outreach (Tier 3–5)

**When to use:** Re-inviting a guest who has appeared on GSR before.

> **Source:** Same `GSR_Outreach_Email_Template_Guide.md`. Tier resolution: Tier 3 = 1 appearance, Tier 4 = 2–4 appearances, Tier 5 = 5+ appearances.

```
You are drafting a returning guest outreach email for Genesis Science Report (GSR).
...
✗ No exclamation points (Tiers 3 — Tier 4/5 may use one if tone fits naturally)
...
## 12 — Booking Confirmation Emails

**When to use:** Once a guest has confirmed their availability and a date/time is set.

> **Source:** `GSR_Guest_Emails_May28-29.txt` — the actual confirmation emails used for Dr. David Legates, Dr. Tim Clarey, Dr. Andrew Fabich.

```
You are writing the 2-email booking confirmation sequence for a confirmed GSR guest.
...
## 13 — Pre-Interview Reminder

**When to use:** Sending a reminder to the guest ~5–7 days before their interview. Replaces the OBSOLETE template.

> **Source:** `OBSOLETE_Reminders_Follow_up_for_Time_and_Talking_Points.docx` — rebuilt with current standards.

```
You are writing a pre-interview reminder email for a GSR guest.
...
## 14 — Post-Interview Thank-You

**When to use:** Following up the day after recording. Replaces the OBSOLETE post-interview template.

> **Source:** `OBSOLETE_Post_Interview_Interview_Next_Day_Follow_Up.docx` — rebuilt with current air-day note.

```
You are writing a post-interview thank-you email for a GSR guest.
...
## 15 — No-Response Follow-Up

**When to use:** No reply received within 5–7 business days of the original cold outreach. Replaces the OBSOLETE no-response template.

```
You are writing a no-response follow-up email for a GSR guest outreach.
...
## 16 — Graphics Assignment

**When to use:** Setting up or reviewing a new episode's graphics tracking sheet.

> **Source:** `05_GSR Graphics Tracking: May` (Drive ID: `1GmdVDOP4h0k6FmOdZLMJNroz7_x4xDcErBYmdGU0890`). Crew assignments are defaults — confirm with Daniel before changing.

```
You are assigning graphics to crew for Genesis Science Report Episode {###}.
...
## 17 — Graphics Sourcing

**When to use:** Researching what images, screenshots, and assets to create or find for a specific segment.

> **Source:** `May GSR - Interview Graphics Paste Doc` (Drive ID: `1wletLhYObVkD34WUYwd0ynCaESzSHxtTQou25You1pQ`) — this is the canonical format used in May 2026 production.

```
You are helping source graphics for a Genesis Science Report segment.
...
```
You are walking me through the episode delivery checklist for Genesis Science Report Episode {###}.
...
☐ 5. SIGNIANT MEDIA SHUTTLE (for Real Life Network / RLN)
   ...
   - RLN thumbnail included: 1200×1800 px portrait (this is different from YouTube)
...
## 19 — Rundown Creator Population

**When to use:** Populating the Graphics and Last Line columns in Rundown Creator from an episode script.

> **Source:** `GSR-Archive_mcp-rundown-creator` and `gsr_pass1.py`. Custom column IDs are fixed.  
> **API note:** RC API requires `RundownID` (capital R and D) — not `rundownId`. This was the source of the MCP timeout bug documented in PR #24.

```
You are parsing a GSR episode script to populate Rundown Creator fields.
...
These are known issues found during the archive scan. Flag them when they come up in production:
...
## Prompts Not Yet Written (Future)

These were identified but need more source data or a dedicated session:

- **Syncing between sources** (Drive ↔ Supabase ↔ Rundown Creator) — needs the RC API full spec
- **Graphics Tracker sync** (auto-populate from Drive paste doc to Supabase) — needs the RC integration to be working first
- **Sponsorship / partnership outreach** — a separate email category from guest outreach; different templates exist in `11_Business & Outreach` folder
- **Viewer Voices curation** — selecting 2 testimonials per episode from submitted viewer messages
- **Kids Corner / GSM / THD script writing** — pre-produced segments with their own format
